...
while k < len(file):
    ...
    for line in rdr:
        ...
        if line[2] == "변기":
            ...
            local_totalplace += 1
        elif (line[2] == "전자렌지") or (line[2] == "식사 판단") or (line[2] == "냉장고") or (line[2] == "식사"):
        ...
        while i < len(local_pill):
            if ((int(local_pill[i-2][12:14]) - int(local_pill[i-1][12:14])) < -1 or (int(local_pill[i-2][12:14]) - int(local_pill[i-1][12:14])) > 1):
                if ((int(local_pill[i-1][12:14]) - int(local_pill[i][12:14])) < -1 or (int(local_pill[i-1][12:14]) - int(local_pill[i][12:14])) > 1):
            ...
            if local_pill_flag == True:
                ...
                local_pill_unbalance = local_pill_unbalance + local_pill[i-2] + local_pill[i-1] + local_pill[i] + " && "
                local_pill_flag = False
    ...
    #store in dictionary
    try:
        outing_density[file[k]] = format((local_outing / 31), '.3f') # store outing density
    except ZeroDivisionError:
        outing_density[file[k]] = 0.000
    
    try:
        activition_score[file[k]] = format((local_activition_score / local_totalactivition), '.5f')
    except ZeroDivisionError:
